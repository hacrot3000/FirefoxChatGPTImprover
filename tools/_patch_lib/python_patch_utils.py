...
- Work with the supervised runner v5 and emit an optional machine-readable result file.
...
PYTHON_PATCH_TOOL_VERSION = "5.16.0"

PYTHON_PATCH_UTILS_RESULT_POLICY = "failed-first/no-change-only-on-0-0-v4-rich-machine-result"
...
def _print_patch_cli_help(patch_name: str) -> None:
    ...
    print()
    print("Options handled by tools/_patch_lib/python_patch_utils.py:")
...
def _write_machine_result(
    ...
    *,
    exit_code: int,
    failed_zip: Optional[Path],
) -> None:
    """Write a best-effort JSON result for the supervising v5 runner."""
    destination = os.environ.get("PYTHON_PATCH_RESULT_FILE", "").strip()
    if not destination:
        return
    payload = {
        "schema_version": 1,
        "tool_version": PYTHON_PATCH_TOOL_VERSION,
        "patch_name": state.patch_name,
        "status": "PASS" if exit_code == 0 else "FAIL",
        "exit_code": exit_code,
        "stats": {
            "patched": state.stats.patched,
            "created": state.stats.created,
            "unchanged": state.stats.unchanged,
            "backups": state.stats.backups,
        ...
        "failed_zip": str(failed_zip.resolve()) if failed_zip else "",
        "failures": [
            {
                "file": exc.rel_path,
                "message": exc.message,
                "op": exc.op_id or "",
                "strategy": exc.strategy or "",
                "anchor": exc.anchor or "",
                "expected": (exc.expected or "")[:50000],
                "context": (exc.context or "")[:50000],
                "candidates": [item[:20000] for item in exc.candidates[:5]],
            }
    ...
    try:
        ...
        print(f"WARNING: cannot write machine-readable patch result: {exc}", file=sys.stderr)
        try:
...
def run_patch(
    ...
    *,
    ...
    print_summary(state)
    failed_zip = maybe_prompt_zip_failed_files(
        state,
        force=prompt_zip_on_error,
        delete_generated_zip=delete_failed_zip,
    )
    exit_code = 1 if state.failures else 0
    _write_machine_result(state, exit_code=exit_code, failed_zip=failed_zip)
    if state.failures:
        RED = "\033[91m"
        BOLD = "\033[1m"
        RESET = "\033[0m"
...
def load_ops_file(path: Path) -> tuple[str, list[dict[str, Any]], dict[str, Any]]:
    """Load a data-only PATCH_TOOL_OPS.json payload.

    This format removes Python boilerplate and syntax risk for patches that fit
    the standard operation DSL.  Unknown top-level fields are rejected so the
    package remains deterministic and easy to review.
    """
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception as exc:
        raise RuntimeError(f"Cannot read patch ops file {path}: {exc}") from exc
    if not isinstance(data, dict):
        raise RuntimeError("PATCH_TOOL_OPS.json must contain one JSON object")
    unsupported = sorted(set(data) - {"schema_version", "patch_name", "default_on_error", "ops"})
    if unsupported:
        raise RuntimeError("Unsupported PATCH_TOOL_OPS.json fields: " + ", ".join(unsupported))
    if data.get("schema_version", 1) != 1:
        raise RuntimeError("Unsupported PATCH_TOOL_OPS.json schema_version")
    patch_name = str(data.get("patch_name", "")).strip()
    if not patch_name:
        patch_name = path.parent.name or "data_patch"
    ops = data.get("ops")
    if not isinstance(ops, list):
        raise RuntimeError("PATCH_TOOL_OPS.json field ops must be a list")
    if any(not isinstance(op, dict) for op in ops):
        raise RuntimeError("Every PATCH_TOOL_OPS.json operation must be an object")
    default_on_error = str(data.get("default_on_error", "stop")).strip().lower()
    if default_on_error not in {"stop", "skip", "ignore"}:
        raise RuntimeError("default_on_error must be stop, skip, or ignore")
    return patch_name, ops, {"default_on_error": default_on_error}


def run_ops_file(path: str | Path) -> int:
    """Apply a data-only patch payload through the same safe run_patch engine."""
    ops_path = Path(path).expanduser().resolve()
    patch_name, ops, options = load_ops_file(ops_path)
    return run_patch(patch_name, ops, default_on_error=options["default_on_error"])
